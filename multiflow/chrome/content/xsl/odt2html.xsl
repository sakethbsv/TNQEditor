...
<!-- due to different style naming with "class" template above -->
  ...
  <xsl:template name="style_standard_default" mode="CSS-attr">
		...
		<xsl:choose>
			<xsl:when test="name()='style:page-layout'">
				...
				<xsl:text>border-right: 2px solid black;</xsl:text>
  ...
  <xsl:template match="@svg:width|@svg:height" mode="CSS-attr">
  ...
  <xsl:template match="@style:width|@fo:width|@fo:height" mode="CSS-attr">
		<xsl:call-template name="copy-attr-normalized"/>
	</xsl:template>
  <xsl:template match="@fo:margin-top" mode="CSS-attr">
		<xsl:choose>
			<xsl:when test="ancestor::style:page-layout">
  ...
  <xsl:template xmlns:exslt="http://exslt.org/common" match="office:spreadsheet//table:table">
		<div>
			<xsl:call-template name="add_id"/>
			<xsl:attribute name="class">
				<xsl:text>masterpage_</xsl:text>
  ...
  <xsl:template xmlns:exslt="http://exslt.org/common" match="table:table" name="table">
		<table>
			<xsl:call-template name="add_id"/>
			<xsl:call-template name="class"/>
...
<!--
			...
			<xsl:variable name="all_columns" select="exslt:node-set($columns)" />
-->
			...
			<xsl:variable name="max_row_width">
				<xsl:for-each select=".//table:table-row">
					...
					<xsl:if test="position()=1"><xsl:value-of select="count(.//table:table-cell[not(last()) or child::*])"/></xsl:if>
				</xsl:for-each>
			</xsl:variable>

			<xsl:variable name="common_columns" select="count(.//table:table-column[not(@table:number-columns-repeated)])"/>
			<xsl:variable name="repeated_columns" select="sum(.//table:table-column[@table:number-columns-repeated and not(last() and @table:number-columns-repeated &gt; 64)]/@table:number-columns-repeated)"/>
			<xsl:variable name="last_bad_columns" select="count(.//table:table-column[last() and @table:number-columns-repeated &gt; 64])"/>
			<xsl:variable name="num_columns" select="$common_columns + $repeated_columns + $last_bad_columns"/>

			<xsl:apply-templates select="*">
<!--				<xsl:with-param name="all_columns" select="$all_columns" /> -->
				<xsl:with-param name="num_cols" select="$num_columns"/>
			</xsl:apply-templates>
		</table>
		<xsl:value-of select="$linebreak"/>
	</xsl:template>
  <xsl:template xmlns:exslt="http://exslt.org/common" name="pre_scan_columns">
		<xsl:choose>
			<!-- do not reproduce "endless spreadsheet" emulation -->
			<xsl:when test="(last() or last()-1) and @table:number-columns-repeated &gt; 64">
				<xsl:copy-of select="."/>
			</xsl:when>
  ...
  <xsl:template xmlns:exslt="http://exslt.org/common" name="repeat_scanned_column">
  ...
  <xsl:template xmlns:exslt="http://exslt.org/common" match="table:table-column-group|table:table-row-group">
  ...
  <xsl:template xmlns:exslt="http://exslt.org/common" match="table:table-header-columns|table:table-columns">
		<xsl:param name="num_cols"/>
  ...
  <xsl:template xmlns:exslt="http://exslt.org/common" match="table:table-header-rows|table:table-rows">
		<xsl:param name="num_cols"/>
		<xsl:param name="visibility" select="'_undefined'"/>
  ...
  <xsl:template xmlns:exslt="http://exslt.org/common" match="table:table-column">
		<xsl:param name="num_cols"/>
		<xsl:param name="visibility" select="'_undefined'"/>
		<col>
			<xsl:call-template name="class"/>
  ...
  <xsl:template xmlns:exslt="http://exslt.org/common" match="table:table-row">
		<xsl:param name="num_cols"/>
		<xsl:param name="visibility" select="'_undefined'"/>
		<xsl:variable name="row_repeat">
			<xsl:choose>
				<!-- do not reproduce "endless spreadsheet" emulation -->
				<xsl:when test="(last() or last()-1) and @table:number-rows-repeated &gt; 64">1</xsl:when>
		...
		<xsl:variable name="def_cell_style">
			<xsl:choose>
...
<!-- from "class" template in css.xml (prepend table-cell_) -->
		...
		<xsl:call-template name="repeat-table-row">
			...
			<xsl:with-param name="def_cell_style" select="$def_cell_style"/>
		</xsl:call-template>
	</xsl:template>
  <xsl:template xmlns:exslt="http://exslt.org/common" name="repeat-table-row">
		<xsl:param name="num_cols"/>
		<xsl:param name="visibility" select="'_undefined'"/>
		<xsl:param name="repeat"/>
		<xsl:param name="def_cell_style" select="''"/>
  ...
  <xsl:template xmlns:exslt="http://exslt.org/common" match="table:table-cell">
		<xsl:param name="num_cols"/>
		<xsl:param name="def_cell_style" select="''"/>
		...
		<xsl:variable name="real_position" select="$common_cells_before + $repeated_cells_before"/>

		<xsl:variable name="cell_repeat">
			<xsl:choose>
				<!-- do not reproduce "endless spreadsheet" emulation -->
				<xsl:when test="(last() or last()-1) and @table:number-columns-repeated &gt; 64"><xsl:value-of select="$num_cols - $real_position + 1"/></xsl:when>
				<xsl:when test="@table:number-columns-repeated &gt; 1">
					<xsl:value-of select="@table:number-columns-repeated"/>
				</xsl:when>
				<xsl:otherwise>1</xsl:otherwise>
  ...
  <xsl:template xmlns:exslt="http://exslt.org/common" name="process-table-cell">
		<xsl:param name="repeat"/>
		<xsl:param name="def_cell_style" select="''"/>
		<xsl:if test="$repeat &gt; 0">
			<td>
				<xsl:call-template name="class">
					<xsl:with-param name="prepend_style">
						<xsl:value-of select="$def_cell_style"/>
					</xsl:with-param>
				</xsl:call-template>
...
<!-- temporary hack: copy cell value into TD title -->
  ...
  <xsl:template xmlns:exslt="http://exslt.org/common" name="column_row_visibility">
		<xsl:param name="visibility" select="'_undefined'"/>
		<xsl:choose>
  ...
  <xsl:template match="office:text">
		...
		<div style="background-color: white;"> <!-- default page background color -->
			<div>
				<xsl:attribute name="class">
					<xsl:text>masterpage_</xsl:text>
					<xsl:value-of select="//style:master-page[1]/@style:name"/>
