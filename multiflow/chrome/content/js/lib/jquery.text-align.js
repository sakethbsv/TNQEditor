...
(function ($) {
	$.extend($.fn, {
		textAlign: function (string) {
			...
			left = $(Editor().contentDocument.createElement("span")).append(str.slice(0, str.indexOf(string)));
			right = $(Editor().contentDocument.createElement("span")).append(str.slice(str.indexOf(string)));
			
			//left = $('<span>' + str.slice(0, str.indexOf(string)) + '</span>');
			//right = $('<span>' + str.slice(str.indexOf(string)) + '</span>');
                    } else {
			left = $(Editor().contentDocument.createElement("span")).append('<span>' + str + '</span>');
			right = $(Editor().contentDocument.createElement("span")).append("<span/>");
                        //left = $('<span>' + str + '</span>');
                        //right = $('<span/>');
                    }
